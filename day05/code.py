...
def compute_part_1(input_file_name="input.txt"):
    ...
    rules = [[int(x[0]), int(x[1])] for x in re.findall(r"(\d+)\|(\d+)", "".join(input))]
    updates = [[int(x.replace('\n', '')) for x in line.split(',')] for line in input if ',' in line]
    def compare(a, b):
        for rule in rules:
            if rule[0] == a and rule[1] == b:
                return -1
            elif rule[1] == b and rule[0] == a:
                return 1
        return 0
    ...
    def compare(a, b):
        for rule in rules:
            if rule[0] == a and rule[1] == b:
                ...
                return 1
        return 0
    return sum([sorted(update, key=cmp_to_key(compare))[len(update)//2] for update in updates if update != sorted(update, key=cmp_to_key(compare))])
