...
# Historians' device! You'll need get it back while The Historians are 
...
def compute_part_1(input_file_name="input.txt"):
    ...
    for k in range(len(secret_numbers)):
        for _ in range(2000):    
            secret_numbers[k] = evolve(secret_numbers[k])
...
def compute_part_2(input_file_name="input.txt"):
    input = read_input(input_file_name)
    secret_numbers = [int(line.replace('\n', '')) for line in input]
    cstb = {} # Maps Change Sequences to Numbers of 
    for secret_number in secret_numbers:
        ...
        for _ in range(2000):
            ...
            # Update local Map
            if len(changes) >= 4:
                change_sequence = str(changes[-4]) + ',' + str(changes[-3]) + ',' + str(changes[-2]) + ',' + str(changes[-1]) 
                if change_sequence not in local_ctsb.keys():
                    local_ctsb[change_sequence] = prices[-1]
        # Update overall Map
        for change_sequence in local_ctsb:
            cstb[change_sequence] = cstb.get(change_sequence, 0) + local_ctsb[change_sequence]
    return max(cstb.values())
